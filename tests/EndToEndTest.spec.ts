...
async function performRegistration(page: Page): Promise<string> {
    ...
    let email: string = randomUtil.getemail();
    ...
    await registerPage.checkprivacypolicy();  // Accept the privacy policy
    await registerPage.clickoncontinue();     // Submit the registration form
    //await page.waitForTimeout(5000);

    // Validate that the registration was successful
    const confirmationMsg = await registerPage.registerconfirmation();
